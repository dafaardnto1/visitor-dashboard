...
function VisitorContent() {
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('realtime-final')
    ...
    <main onClick={() => setIsAudioReady(true)} className="min-h-screen bg-[#F1F5F9] p-4 md:p-12 font-sans text-slate-900 selection:bg-blue-600">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-12">
        ...
        <section className="lg:col-span-1">
          <div className="bg-white p-8 md:p-10 rounded-[3rem] shadow-2xl border border-white lg:sticky lg:top-12">
        ...
        <section className="lg:col-span-2">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-6 px-4">
            ...
            <div className="relative w-full md:w-72">
              <input type="text" placeholder="Search visitor..." className="w-full pl-14 pr-6 py-4 bg-white rounded-[1.5rem] text-xs font-black border-none shadow-xl outline-none focus:ring-2 focus:ring-blue-600 transition-all uppercase tracking-widest" onChange={(e) => setSearchTerm(e.target.value)} />
            </div>
          </div>
              ...
              <table className="w-full text-left min-w-[850px]">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                    <th className="px-8 py-10 text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Time / Status</th>
                    <th className="px-8 py-10 text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Checkout</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {filteredVisitors.map((v) => (
                    <tr key={v.id} className={`group transition-all duration-300 ${v.status === 'waiting' ? 'bg-red-50/20' : v.jam_keluar ? 'opacity-60 grayscale-[0.5]' : ''}`}>
                      ...
                      <td className="px-8 py-10">
                        {v.jam_keluar ? <div className="text-xs font-black text-slate-500 bg-slate-100 px-3 py-1.5 rounded-lg w-fit">{new Date(v.jam_keluar).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</div> : <span className="text-[10px] font-bold text-slate-300 uppercase tracking-widest italic">On-site</span>}
                      </td>
                      <td className="px-8 py-10">
                        <div className="text-lg font-black text-slate-900 uppercase leading-tight mb-1">{v.nama}</div>
                        <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{v.instansi || 'Independent'}</div>
                      </td>
                      {/* KOLOM PURPOSE YANG KEMBALI */}
                      <td className="px-8 py-10">
                      ...
                      {isAdmin && (
                        <td className="px-8 py-10 text-right">
                          <div className="flex justify-end items-center gap-3">
                            {v.status === 'waiting' && <button onClick={() => handleAcc(v.id)} className="px-4 py-2 bg-green-500 text-white text-[9px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-900 transition-all shadow-lg italic">Verify</button>}
                            {v.status === 'done' && !v.jam_keluar && <button onClick={() => handleCheckout(v.id)} className="px-4 py-2 bg-blue-500 text-white text-[9px] font-black uppercase tracking-widest rounded-xl hover:bg-slate-900 transition-all shadow-lg italic">Checkout</button>}
