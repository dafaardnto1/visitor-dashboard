...
function VisitorContent() {
  ...
  const playNotification = () => {
    try {
      ...
      // .catch di sini kunci agar mobile tidak memunculkan SecurityError
      audio.play().catch(() => console.log("Audio waiting for user tap..."));
    } catch (e) {
      console.error("Audio error ignored");
    }
  };

  useEffect(() => {
    const fetchVisitors = async () => {
      // Selalu ambil data terbaru (descending)
      const { data } = await supabase.from('visitors').select('*').order('jam_masuk', { ascending: false });
      if (data) setVisitors(data);
    };
    fetchVisitors();

    const channel = supabase.channel('stable-realtime')
  ...
  return (
    <main className="min-h-screen bg-[#F1F5F9] p-4 md:p-12 font-sans text-slate-900">
      ...
      {/* POPUP: SUCCESS */}
      {showSuccessPopup && (
        <div className="fixed inset-0 flex items-center justify-center z-[200] px-4 animate-in fade-in duration-300">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-md"></div>
          <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-white text-center relative z-[201] max-w-sm w-full animate-in zoom-in duration-300">
      ...
      {deleteConfirm.show && (
        <div className="fixed inset-0 flex items-center justify-center z-[200] px-4">
          <div className="absolute inset-0 bg-slate-900/60 backdrop-blur-md"></div>
          <div className="bg-white p-10 rounded-[3rem] shadow-2xl border border-white text-center relative z-[201] max-w-sm w-full animate-in zoom-in duration-200">
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <section className="lg:col-span-1">
          <div className="bg-white p-8 md:p-10 rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl border border-white lg:sticky lg:top-12">
        ...
        <section className="lg:col-span-2">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 gap-6 px-4">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 bg-white px-4 py-2 rounded-full border border-slate-100 shadow-sm w-fit">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic tracking-wider">Active Monitoring System</span>
              </div>
            </div>
            <input type="text" placeholder="Search visitor..." className="w-full md:w-72 pl-8 pr-6 py-4 bg-white rounded-2xl text-xs font-black border-none shadow-xl outline-none focus:ring-2 focus:ring-blue-600 transition-all uppercase" onChange={(e) => setSearchTerm(e.target.value)} />
          </div>

          <div className="bg-white rounded-[3rem] md:rounded-[4rem] shadow-2xl border border-white overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left min-w-[700px]">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                    <th className="px-10 py-8 text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Timestamp</th>
                    <th className="px-10 py-8 text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Visitor Profile</th>
                    <th className="px-10 py-8 text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Purpose</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {filteredVisitors.map((v) => (
                    <tr key={v.id} className={`group transition-all duration-300 ${v.status === 'waiting' ? 'bg-red-50/20' : v.jam_keluar ? 'opacity-50 grayscale-[0.3]' : ''}`}>
                      ...
                      {isAdmin && (
                        <td className="px-10 py-8 text-right flex justify-end gap-3 items-center min-h-[100px]">
